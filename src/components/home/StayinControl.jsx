    ...
    const items = [
        {
            icon: (<svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="40" height="40" rx="8" fill="url(#paint0_linear_11881_278536)" />
                <path d="M20 13.125V20M20 20V26.875M20 20H13.125M20 20H26.875" stroke="white" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round" />
                <defs>
                    <linearGradient id="paint0_linear_11881_278536" x1="25.9592" y1="5.66667" x2="55.4034" y2="18.6037" gradientUnits="userSpaceOnUse">
        ...
        {
            icon: (<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M9.99984 8.33464C11.8408 8.33464 13.3332 6.84225 13.3332 5.0013C13.3332 3.16035 11.8408 1.66797 9.99984 1.66797C8.15889 1.66797 6.6665 3.16035 6.6665 5.0013C6.6665 6.84225 8.15889 8.33464 9.99984 8.33464Z" stroke="white" strokeWidth="1.25" />
                <path d="M14.9995 7.50065C16.3803 7.50065 17.4995 6.56732 17.4995 5.41732C17.4995 4.26732 16.3803 3.33398 14.9995 3.33398M4.99951 7.50065C3.61868 7.50065 2.49951 6.56732 2.49951 5.41732C2.49951 4.26732 3.61868 3.33398 4.99951 3.33398" stroke="white" strokeWidth="1.25" strokeLinecap="round" />
                <path d="M9.99951 17.5007C12.7609 17.5007 14.9995 16.0083 14.9995 14.1673C14.9995 12.3264 12.7609 10.834 9.99951 10.834C7.23809 10.834 4.99951 12.3264 4.99951 14.1673C4.99951 16.0083 7.23809 17.5007 9.99951 17.5007Z" stroke="white" strokeWidth="1.25" />
                <path d="M16.6663 15.8346C18.128 15.5138 19.1663 14.7021 19.1663 13.7513C19.1663 12.8005 18.128 11.9888 16.6663 11.668M3.33301 15.8346C1.87134 15.5138 0.833008 14.7021 0.833008 13.7513C0.833008 12.8005 1.87134 11.9888 3.33301 11.668" stroke="white" strokeWidth="1.25" strokeLinecap="round" />
            </svg>),
            title: 'Social media',
            des: 'Provide customers with best-in-class chat support by using the suggested SLA settings for chat.',
            btn: ''
            ...
            <div className="grid grid-cols-1 gap-6 p-4 md:p-5 lg:p-6 ">
                <div className="bg-primary/10 rounded-2xl md:rounded-[20px] p-4 md:p-6 text-center">
                    <div className="mx-auto max-w-[500px]">
                        <h4 className='text-heading font-semibold text-xl md:text-2xl lg:text-3xl xl:text-[34px] !leading-[140%] mb-3 md:mb-4 lg:mb-5'>Get started with service level agreements (SLAs)</h4>
                        <p className='text-sm font-medium text-heading !leading-normal'>Service Level Agreements (SLAs) boost customer satisfaction, enhance accountability, optimize resource allocation, streamline problem resolution, and minimize disputes.</p>
                    </div>
                </div>
                <div className="flex flex-wrap gap-4 md:gap-6">
                    {items.map((item, index) => (
                        <div className="group flex flex-col p-4 rounded-xl border border-solid border-stroke hover:border-primary bg-white hover:bg-primary/5 transition-all duration-300 max-w-[260px] min-w-full md:min-w-[260px]" key={index}>
                            <div className="size-10 rounded-lg bg-btn text-white flex items-center justify-center mb-3">
                                {item.icon}
                            </div>
                            <h4 className='font-semibold text-base mb-1'>{item.title}</h4>
                            <p className='font-medium text-gray text-xs !leading-[140%]'>{item.des}</p>
                            <div className="mt-auto">
                                <Link to={item.btn.url || ''} className='btn w-full mt-4 capitalize border-primary text-primary'>{item.btn.text || 'use Template'}</Link>
                            </div>
                        </div>
                    ))}
                </div>
