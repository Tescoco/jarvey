...
export default function Test() {
  // State management
  const [activeTab, setActiveTab] = useState('mail'); // 'mail' or 'message'
  ...
  const handleNewConversation = () => {
    ...
    alert('New conversation started! All fields have been cleared.');
  };
  ...
  return (
    <div className='mt-3 lg:mt-4 flex justify-center'>
      <div className='w-full md:w-[712px] lg:w-full'>
        <div className='mb-12 lg:mb-20 xl:mb-[100px] 2xl:mb-[120px] 3xl:mb-[162px] w-full'>
          <div className='mb-3 lg:mb-4 flex gap-3 lg:gap-4 w-full'>
            <img src={img} className='w-8 lg:w-10 h-8 lg:h-10 object-contain' alt="" />
            <div className='bg-[#F8F7FF] rounded-lg lg:rounded-xl xl:rounded-2xl p-3 lg:p-4 w-full'>
              <p className='text-sm font-semibold !leading-[1.5] text-[#0A0D14] mb-2 lg:mb-3'>Jarvey AI</p>
              <p className='text-[#0A0D14] text-xs !leading-[1.6] mb-3 lg:mb-4'>Hi Mian!</p>
              <p className='text-[#0A0D14] text-xs !leading-[1.6] mb-3 lg:mb-4'>Welcome to your Al Agent test area.</p>
              <p className='text-[#0A0D14] text-xs !leading-[1.6] mb-3 lg:mb-4'>Your test area lets you search for an existing customer to see how your Al Agent would respond based on your resources and the customer's order history.</p>
            </div>
          </div>
          <div className='mb-3 lg:mb-4 flex gap-3 lg:gap-4 w-full'>
            <img src={img} className='w-8 lg:w-10 h-8 lg:h-10 object-contain' alt="" />
            <div className='bg-[rgba(254,67,51,0.06)] rounded-lg lg:rounded-xl xl:rounded-2xl p-3 lg:p-4 w-full'>
              <p className='text-sm font-semibold !leading-[1.5] text-[#0A0D14] mb-2 lg:mb-3'>Jarvey AI</p>
              <p className='text-[#0A0D14] text-xs !leading-[1.6] mb-2 '>Welcome to your Al Agent test area.</p>
              <p className='text-[#0A0D14] text-xs font-semibold !leading-[1.5] mb-2'>How did it do? 👍 Good 👎 Bad</p>
              <p className='text-[#0A0D14] text-xs  !leading-[1.6] mb-2'>The Al Agent used the following Guidance:</p>
              <p className='text-[#0A0D14] text-xs  !leading-[1.6] mb-2 pl-4 lg:pl-5'>When a customer asks for a discount or promotion</p>
            </div>
          </div>
        </div>
        <div className='bg-[#F7F7F7] rounded-lg lg:rounded-xl xl:rounded-2xl mb-4 lg:mb-5'>
          <div className='md:flex items-center justify-between py-2 px-4'>
            <div className='mb-3 lg:mb-0'>
              <button
                onClick={() => handleTabSwitch('mail')}
                className={`${activeTab === 'mail' ? '!bg-white' : ''} text-[#0A0D14] text-xs font-medium !leading-[1.5] py-2 lg:py-[9px] px-6 lg:px-7 rounded-xl transition-colors`}
              >
                Mail
              </button>
              <button
                onClick={() => handleTabSwitch('message')}
                className={`${activeTab === 'message' ? '!bg-white' : ''} text-[#0A0D14] text-xs font-medium !leading-[1.5] py-2 lg:py-[9px] px-6 lg:px-7 rounded-xl transition-colors`}
              >
                Message
              </button>
            </div>
          ...
          <div className='px-3 lg:px-4'>
            <div className='text-[#0A0D14] text-sm font-medium !leading-[1.4] tracking-[-0.084px] border-y border-y-[#E2E4E9] py-2 lg:py-3 flex items-center gap-2'>
              <span>Subject:</span>
              <input
                type="text"
                value={subject}
                onChange={handleSubjectChange}
                className='text-xs text-gray font-normal !leading-[1.66] bg-transparent flex-1 outline-none'
            ...
            <div className='py-3 lg:py-4'>
              <textarea
                ...
                className='text-gray text-xs font-medium !leading-[1.66] mb-1 bg-transparent w-full resize-none placeholder:text-gray h-20 overflow-y-auto outline-none'
                placeholder='Enter your questions'
              />
              <p className='text-heading text-sm font-medium !leading-[1.42] mb-2 lg:mb-3'>or test a common question</p>
              <div className="flex items-center flex-wrap gap-3 mb-4 md:mb-5">
                {autoAnswer.map((item, index) => (
                  <button
                    key={index}
                    onClick={() => handleQuickQuestion(item)}
                    className='block font-medium font-inter text-sm bg-white rounded-lg px-3 py-2 text-heading w-fit hover:bg-gray-50 transition-colors'
                  >
                    {item}
                  </button>
                ))}
              ...
              <ul className="flex items-center gap-2 py-2">
                {textEditor.map((item, index) => (
                  <li key={index}>
                    <button
                      onClick={() => handleEditorAction(index)}
                      className='text-[#111111]/50 hover:text-primary transition-colors'
                    >
                      {item.icon}
                    </button>
                  </li>
        ...
        <div className='flex items-center gap-3 lg:gap-4'>
          <button
            onClick={handleSend}
            className='btn !bg-[#7E5EFF] !text-white hover:bg-[#6B4FE6] transition-colors'
          >
            Send
          </button>
          <button
            onClick={handleNewConversation}
            className='btn text-primary hover:bg-primary/10 transition-colors'
          >
            New Conversation
          </button>
        </div>
...
//           <div className='md:flex items-center justify-between py-2 px-4'>
//             <div className='mb-3 lg:mb-0'>
//               <button className='!bg-white text-[#0A0D14] text-xs font-medium !leading-[1.5] py-2 lg:py-[9px] px-6 lg:px-7 rounded-xl '>Mail</button>
//               <button className='text-[#0A0D14] text-xs font-medium !leading-[1.5] py-2 lg:py-[9px] px-6 lg:px-7'>Message</button>
//             </div>
//             <Dropdown info='New Text' className='!mb-0' placeholder="Select user " isArrow={true} items={[
//               { name: "DropDown1" },
//               { name: "DropDown2" },
//               { name: "DropDown3" },
//               { name: "DropDown4" },
...
//           </div>
//           <div className='px-3 lg:px-4'>
//             <p className='text-[#0A0D14] text-sm font-medium !leading-[1.4] tracking-[-0.084px] border-y border-y-[#E2E4E9] py-2 lg:py-3'>Subject: <span className='text-xs text-gray font-normal !leading-[1.66]'>Type here</span></p>
//             <div className='py-3 lg:py-4'>
//               <textarea className='text-gray text-xs font-medium !leading-[1.66] mb-1 bg-transparent w-full resize-none placeholder:text-gray h-20 overflow-y-auto' placeholder='Enter your questions' />
//               <p className='text-heading text-sm font-medium !leading-[1.42] mb-2 lg:mb-3'>or test a common question</p>
//               <div className="flex items-center flex-wrap gap-3 mb-4 md:mb-5">
//                 {autoAnswer.map((item, index) => (
//                   <button key={index} className='block font-medium font-inter text-sm bg-white rounded-lg px-3 py-2 text-heading w-fit'>{item}</button>
//                 ))}
//               </div>
//               <ul className="flex items-center gap-2 py-2">
//                 {textEditor.map((item, index) => (
...
//         </div>
//         <div className='flex items-center gap-3 lg:gap-4'>
//           <button className='btn !bg-[#7E5EFF] !text-white'>Send</button>
//           <button className='btn text-primary'>New Conversion</button>
//         </div>
//       </div>
//     </div>
//   )
