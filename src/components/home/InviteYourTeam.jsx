...
export default function InviteYourTeam() {
  ...
  const handleCancel = () => {
    // Navigate back or to the overview page
  ...
  const handleInvite = () => {
    ...
    // Validate that we have at least one valid agent
    if (payload.length === 0) {
      alert("Please add at least one agent with name, email, and role.");
      return;
    }

    // Validate each agent has required fields
    const invalidAgents = payload.filter((a) => !a.name || !a.email || !a.roleId);
    if (invalidAgents.length > 0) {
      alert("Please fill in all required fields (Name, Email, and Role) for each agent.");
      return;
    }

    // Submit the agents array to your API
    console.log("Inviting agents:", payload);

    // Show success message or navigate
    alert(`Successfully invited ${payload.length} agent(s)!`);

    // Navigate to next step or dashboard
    if (window.location.pathname.includes("/onboarding")) {
      // Let the OnboardingWrapper handle navigation
      window.dispatchEvent(new CustomEvent('invite-complete'));
    }
  };
    ...
    <div>
      <div>
        ...
        <div className="flex items-center justify-between mt-6">
          <button
            type="button"
            className="btn text-primary border-primary min-w-[74px] px-0"
