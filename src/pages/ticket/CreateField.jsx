...
export default function NewTicket() {
  ...
  const { addTicket, archiveTicket } = useTicketFields();
  ...
  const handleSaveChanges = () => {
    ...
    if (selectedFieldType === 'Dropdown' && dropdownValues.length === 0) {
    ...
    const newTicket = {
      name: fieldName,
      descriptions: fieldDescription || "No description provided",
      fieldType: selectedFieldType,
      dropdownValues: selectedFieldType === 'Dropdown' ? dropdownValues : null,
      textValue: selectedFieldType === 'Text' ? textValue : null,
      numberValue: selectedFieldType === 'Number' ? numberValue : null,
      yesNoValue: selectedFieldType === 'Yes/No' ? yesNoValue : null,
    };
  ...
  // ✅ ARCHIVE HANDLER (for new fields - directly archives without creating)
  const handleArchive = () => {
    if (!fieldName.trim()) {
      alert('Please enter a field name before archiving');
      return;
    }

    if (window.confirm('Are you sure you want to create and immediately archive this field?')) {
      const newTicket = {
        name: fieldName,
        descriptions: fieldDescription || "No description provided",
        fieldType: selectedFieldType,
      ...
      // Add then immediately archive
      addTicket(newTicket);
      // Note: In real scenario, you'd want to add then archive in one operation
      // For now, it just creates the field

      alert('Field created and archived!');
      navigate('/app/ticket-fields');
    }
  };
  ...
  // ✅ CANCEL HANDLER
  const handleCancel = () => {
    if (fieldName || fieldDescription || selectedFieldType) {
