...
import { useTicketFields } from "../../contexts/TicketFieldsContext";
...
export default function TicketFields() {
  ...
  const handlePush = () => {
    if (archiveModal) {
      archiveTicket(archiveModal);
      setShowModal(false);
      setArchiveModal(null);
    }
  };

  // Restore handler - THIS WAS MISSING!
  const handleRestore = (item) => {
    console.log("Restoring item:", item);
    setActiveTable((prev) => [...prev, item]);
    setArchiveTicket((prev) =>
      prev.filter((ticket) => ticket.name !== item.name)
    );
  };
  ...
  //     const [movedItem] = newTickets.splice(draggedItem.index, 1);
  //     newTickets.splice(dropIndex, 0, movedItem);
  //     setActiveTable(newTickets);
  //   } else {
  //     const newTickets = [...archiveTicket];
  //     const [movedItem] = newTickets.splice(draggedItem.index, 1);
  //     newTickets.splice(dropIndex, 0, movedItem);
  //     setArchiveTicket(newTickets);
  //   }
  ...
  const handleDrop = (e, dropIndex) => {
    ...
    if (activeTab === "active") {
      ...
      const [movedItem] = newTickets.splice(draggedItem.index, 1);
      newTickets.splice(dropIndex, 0, movedItem);
      updateActiveTickets(newTickets);
    } else {
      const newTickets = [...archivedTickets];
      const [movedItem] = newTickets.splice(draggedItem.index, 1);
      newTickets.splice(dropIndex, 0, movedItem);
      updateArchivedTickets(newTickets);
    }
  ...
  const filteredActiveTickets = activeTickets.filter(
    (ticket) =>
      ticket.name.toLowerCase().includes(searchQuery) ||
      ticket.descriptions.toLowerCase().includes(searchQuery)
  );

  const filteredArchiveTickets = archivedTickets.filter(
    (ticket) =>
      ticket.name.toLowerCase().includes(searchQuery) ||
      ticket.descriptions.toLowerCase().includes(searchQuery)
  );

  // const filteredActiveTickets = activeTicket.filter(
  //   (ticket) =>
  //     ticket.name.toLowerCase().includes(searchQuery) ||
  //     ticket.descriptions.toLowerCase().includes(searchQuery)
  // );

  // const filteredArchiveTickets = archiveTicket.filter(
  //   (ticket) =>
  //     ticket.name.toLowerCase().includes(searchQuery) ||
  //     ticket.descriptions.toLowerCase().includes(searchQuery)
  // );
              ...
              <div className="border-b border-solid border-stroke flex items-center overflow-x-auto mb-5 md:mb-6">
                {["active", "archive"].map((item, index) => (
                  <button
                    ...
                    key={index}
                    className={`grow capitalize font-inter font-medium text-sm px-4 md:px-5 lg:px-6 pb-3 border-b border-solid ${item === activeTab
                      ? "border-btn text-btn"
              ...
              <table className="w-max lg:w-full">
                  ...
                  <tr className="bg-[#F6F8FA]">
                    {["Name", "Descriptions", "Last Update", "Actions"].map(
                      (item, index) => (
                        <th
                          key={index}
                          className={`text-left p-[8px_12px] text-[#525866] font-medium !leading-[1.42] ${index === 0 && "pl-[52px]"
                            }`}
                        >
                          {item}
                        </th>
                ...
                <tbody>
                  {filteredActiveTickets.length ? (
                    filteredActiveTickets.map((item, index) => (
                      <tr
                        key={index}
                        draggable
                        ...
                        className={`border-b border-[#E2E4E9] transition-all duration-200 ${dragOverIndex === index ? "bg-blue-50" : ""
                      ...
                      >
                        ...
                        >
                          <button
                            ...
                            className="pl-4 hover:text-primary transition-colors duration-200"
                          >
                            {archive}
                          </button>
                        </td>
              ...
              <table className="w-max lg:w-full">
                ...
                <tbody>
                  {filteredArchiveTickets.length ? (
                    filteredArchiveTickets.map((item, index) => (
                      <tr
                        key={index}
                        draggable
                        onDragStart={(e) => handleDragStart(e, item, index)}
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="pl-4 hover:text-green-600 transition-colors duration-200"
                    ...
                    <tr>
                      <td colSpan={4} className="text-center py-8 text-gray">
                        {searchQuery
                          ? "No archived tickets found matching your search"
                          : "No archive tickets"}
                      </td>
                    </tr>
                  )}
        ...
        <Modal closeIconHide={false}>
          <div>
            ...
            {ModalList.map((item, idx) => (
              <p
                key={idx}
                className="text-[#858585] text-sm font-semibold !leading-[1.5] mb-5 lg:mb-6 xl:mb-[30px]"
              >
                {item}
              </p>
            ))}
        ...
        <Modal closeIconHide={false} innerClass="max-w-[400px]">
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <button
              ...
              className="btn bg-red-500 hover:bg-red-600 border-red-500 text-white min-w-max"
            >
              Delete
            </button>
          </div>
