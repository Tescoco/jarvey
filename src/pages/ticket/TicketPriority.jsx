...
export default function TicketPriority() {
  ...
  // Filter the table data based on search
  const filteredData = useMemo(() => {
    if (!searchTerm) return modalTable;

    return modalTable.filter((item) =>
      item.priority.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.response.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.resolve.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [modalTable, searchTerm]);
  ...
  return (
    <div>
      ...
      <div className="p-4 lg:p-5">
        {/* Use the fixed TableFilter component with search functionality */}
        <TableFilter
                ...
                filteredData.map((item, index) => (
                  <tr key={item.id} className="border-b border-[#E2E4E9]">
                    ...
                    <td className="px-3 py-2 md:py-3.5">
                      <span
                        className={`py-1 md:py-1.5 px-3 font-medium text-white rounded-full !leading-[142%] ${item.priority === "Low"
                            ? "bg-[#176448]"
                    ...
                    <td className="px-3 py-2 md:py-3.5">
                      <div className="flex items-center gap-3">
                        <span className="py-1 px-3 font-medium text-primary bg-primary/10 rounded-full !leading-[142%]">
                          Response in: {item.response}
                        </span>
                        <span className="py-1 px-3 font-medium text-[#176448] bg-[#176448]/10 rounded-full !leading-[142%]">
