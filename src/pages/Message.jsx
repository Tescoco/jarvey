    ...
    <>
      ...
      <div className="flex">
        <div className="left max-w-[296px] border-r border-solid border-stroke hidden md:block">
          <Input className='p-3' type="text" placeholder="Search..." leftIcon={search} inputClass='!h-[36px]' />
          <div className="max-h-[calc(100vh-140px)] lg:max-h-[calc(100vh-150px)] overflow-y-auto">
            ...
            {messagesList.map((item, index) => (
              <div className="mb-0 w-full" key={index}>
                <button onClick={() => { autoScroll(); setActiveMessageTab(item); }} className={`w-full flex items-center text-left gap-2 py-4 px-3 border-b border-solid border-[#E2E4E9] border-l-2 ${item.name === activeMessageTab.name ? 'bg-gradient-to-tr from-[#F4F2FF] to-white border-l-primary' : 'bg-transparent border-l-transparent'}`} key={index}>
                  <img src={item.img} alt="" />
                  <p className='text-sm text-heading font-semibold !leading-[150%] line-clamp-1'>{item.name}</p>
                </button>
              </div>
            ))}
          </div>
        ...
        <div className='w-full'>
          <div className={`${c_16} border-t-0 border-l-0 flex flex-wrap items-center justify-between gap-y-3 !rounded-none`}>
            ...
            <div className="flex items-center gap-3">
              {topBtn.map((item, index) => (
                <div className="" key={index}>
                  <button className='size-6 rounded-lg border border-solid border-[#E2E4E9] flex items-center justify-center hover:scale-105 hover:border-primary'>{item.icon}</button>
                </div>
              ))}
            </div>
          </div>
          <div ref={scrollRef} className="overflow-y-auto h-[calc(100vh-189px)] md:h-[calc(100vh-217px)] lg:h-[calc(100vh-225px)] p-4">
              ...
              activeMessageTab.conversions?.map((item, index) => (
                <div className="" key={index}>
                  ...
                  {item.messages.map((mes, i) => (
                    <div className={`p-3 max-w-[456px] rounded-xl mb-3 ${mes.status === 'to' ? 'ml-auto bg-primary' : 'mr-auto bg-[#F7F7F7]'}`} key={i}>
                      <h6 className={`text-xs font-semibold ${mes.status === 'to' ? 'text-right text-white' : 'text-left'}`}>{mes.status === 'to' ? 'ME' : activeMessageTab.name}</h6>
                      <p className={`text-xs font-medium mt-1 !leading-normal ${mes.status != 'to' ? 'text-gray' : 'text-white'}`}>{mes.text}</p>
                    </div>
                  ))}
                </div>
              ))
              :
              <p className='text-sm font-medium bg-gray/5 rounded-2xl font-inter text-center flex h-full justify-center items-center text-heading capitalize'>no more conversions</p>
            }
          </div>
          <div className="relative z-[1] w-full p-4 bg-[#F8F7FF]">
            ...
            <button className='bg-primary text-white flex items-center justify-center rounded-lg size-9 absolute top-1/2 -translate-y-1/2 right-[22px]'>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M3.33313 8L2.93046 4.37533C2.81513 3.338 3.88313 2.576 4.82646 3.02333L12.7891 6.79533C13.8058 7.27666 13.8058 8.72333 12.7891 9.20466L4.82646 12.9773C3.88313 13.424 2.81513 12.6627 2.93046 11.6253L3.33313 8ZM3.33313 8H7.9998" stroke="currentColor" strokeWidth="1.24" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
          </div>
        </div>
