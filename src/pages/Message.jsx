...
export default function Message() {
  ...
  // Update filtered messages when messagesList changes
  useEffect(() => {
    if (searchQuery.trim() === "") {
  ...
  const handleSendMessage = () => {
    ...
    const activeIndex = updatedMessages.findIndex(
      msg => msg.name === activeMessageTab.name
    );

    if (activeIndex !== -1) {
      if (!updatedMessages[activeIndex].conversions) {
        updatedMessages[activeIndex].conversions = [];
      }
      ...
      const lastConversion = updatedMessages[activeIndex].conversions[
        updatedMessages[activeIndex].conversions.length - 1
      ];

      if (lastConversion) {
        lastConversion.messages.push(newMsg);
      } else {
        updatedMessages[activeIndex].conversions.push({
      ...
      setActiveMessageTab(updatedMessages[activeIndex]);
      setNewMessage("");
  ...
  const handleDeleteConversation = () => {
    const updatedMessages = messagesList.filter(
      msg => msg.name !== activeMessageTab.name
    );

    setMessagesList(updatedMessages);
    setFilteredMessages(updatedMessages);
    setActiveMessageTab(updatedMessages[0] || {});
  ...
  const handleMemberSearch = (e) => {
    setMemberSearchQuery(e.target.value.toLowerCase());
  };
  ...
  const getFilteredMembersForModal = () => {
    return messagesList.filter(member =>
  ...
  const topBtn = [
    ...
    },
    {
      icon: (
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M7.99958 9C7.66295 9.88295 6.89415 10.5 5.99958 10.5C5.10502 10.5 4.33622 9.88295 3.99958 9M3.14177 9H8.8574C9.46219 9 9.92856 8.46732 9.84863 7.86784L9.39607 4.47368C9.1691 2.77135 7.71698 1.5 5.99958 1.5C4.28219 1.5 2.83007 2.77135 2.60309 4.47368L2.15054 7.86784C2.07061 8.46732 2.53698 9 3.14177 9Z" stroke="#888888" strokeLinejoin="round" />
        </svg>
      ),
  ...
  return (
    <>
            ...
            {filteredMessages.map((item, index) => (
              <div className="mb-0 w-full" key={index}>
                <button
                  ...
                  className={`w-full flex items-center text-left gap-2 py-4 px-3 border-b border-solid border-[#E2E4E9] border-l-2 ${item.name === activeMessageTab.name
                ...
                >
                  <span className="relative inline-block">
                    ...
                    <div
                      ...
                      className={`absolute bottom-0 left-0 size-2.5 rounded-full border border-white ${isUserOnline(item.name) ? "bg-green-500" : "bg-red-500"
                        }`}
                    />
                  </span>
        ...
        <div className="w-full">
            ...
            <div className="flex items-center gap-2">
              <span className="relative inline-block">
                ...
                <div
                  ...
                  className={`absolute bottom-0 left-0 size-2.5 rounded-full border border-white ${isUserOnline(activeMessageTab?.name || "Marvin McKinney")
                    ? "bg-green-500"
                    : "bg-red-500"
                    }`}
                />
              ...
              {topBtn.map((item, index) => (
                <div className="" key={index}>
                  <button
                    ...
                    className="size-6 rounded-lg border border-solid border-[#E2E4E9] flex items-center justify-center hover:scale-105 hover:border-primary"
                  >
                    {item.icon}
                  </button>
                </div>
              ...
              activeMessageTab.conversions?.map((item, index) => (
                <div className="" key={index}>
                  ...
                  {item.messages.map((mes, i) => {
                    ...
                    return (
                      <div
                        className={`p-3 max-w-[456px] rounded-xl mb-3 ${mes.status === "to"
                          ? "ml-auto bg-primary"
                          : "mr-auto bg-[#F7F7F7]"
                          }`}
                        key={i}
                      >
                        <h6
                          className={`text-xs font-semibold ${mes.status === "to"
                        ...
                        <p
                          className={`text-xs font-medium mt-1 !leading-normal ${mes.status != "to" ? "text-gray" : "text-white"
                            }`}
                        >
                          {mes.text}
                        </p>
                        <div
                          className={`mt-1 flex ${mes.status === "to"
                            ? "justify-end"
                            : "justify-start"
                            }`}
                        >
                          <span
                            className={`text-[10px] ${mes.status === "to"
                              ? "text-white/70"
                              : "text-[#858585]"
                              }`}
                          >
          ...
          <div className="relative z-[1] w-full p-4 bg-[#F8F7FF]">
            ...
            <button
              ...
              className={`flex items-center justify-center rounded-lg size-9 absolute top-1/2 -translate-y-1/2 right-[22px] transition-all ${newMessage.trim()
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Add members to this conversation to collaborate
            </p>
            ...
            <div className="space-y-2 mb-6 max-h-[300px] overflow-y-auto">
              {getFilteredMembersForModal().length > 0 ? (
                getFilteredMembersForModal().map((member, index) => (
                  <button
                    key={index}
                    onClick={() => toggleMemberSelection(member.name)}
                    className="w-full flex items-center gap-3 p-3 rounded-lg border border-stroke hover:bg-gray-50 transition-colors"
                  >
                    <img src={member.img} alt="" className="w-10 h-10 rounded-full" />
              ...
              <button
                onClick={() => {
                  if (selectedMembers.length > 0) {
                    alert(`Added ${selectedMembers.length} member(s): ${selectedMembers.join(', ')}`);
                    setShowAddMemberModal(false);
                    setMemberSearchQuery("");
                    setSelectedMembers([]);
                  } else {
                ...
                className="flex-1 bg-primary text-white rounded-lg py-2 font-medium hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {/* {showAddMemberModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            ...
            <div className="space-y-2 mb-6 max-h-[300px] overflow-y-auto">
              {messagesList.slice(0, 5).map((member, index) => (
                <button
                  key={index}
                  className="w-full flex items-center gap-3 p-3 rounded-lg border border-stroke hover:bg-gray-50 transition-colors"
                >
                  ...
                  <div className="w-5 h-5 rounded border-2 border-gray-300 hover:border-primary"></div>
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-primary text-white rounded-lg py-2 font-medium hover:opacity-90"
              >
                Add Members
              </button>
              <button
      ...
      {showNotificationModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto">
              {[
                ...
                },
                {
                  type: 'mention',
                  user: 'Guy Hawkins',
                  text: 'Mentioned you in a conversation',
                  time: '3 hours ago',
                  unread: false
                },
                {
            ...
            <button
              onClick={() => setShowNotificationModal(false)}
              className="w-full mt-4 border border-stroke rounded-lg py-2 font-medium hover:bg-gray-50"
            >
              Close
            </button>
          </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            <h3 className="text-xl font-semibold mb-2">Delete Conversation</h3>
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete this conversation with{" "}
              <span className="font-semibold">{activeMessageTab.name}</span>?
              This action cannot be undone.
            </p>
            <div className="flex gap-3">
              <button
                onClick={handleDeleteConversation}
                className="flex-1 bg-red-500 text-white rounded-lg py-2 font-medium hover:bg-red-600 transition-colors"
              >
                Delete
              </button>
              <button
                onClick={() => setShowDeleteConfirm(false)}
                className="flex-1 border border-stroke rounded-lg py-2 font-medium hover:bg-gray-50 transition-colors"
              >
                Cancel
              </button>
            </div>
