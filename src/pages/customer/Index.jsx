...
export default function Index() {
  ...
  // State management
  const [emailTable, setEmailTable] = useState(initialEmailTable);
  ...
  const [sortBy, setSortBy] = useState("name"); // name, email, phone, store, date

  // FIX 4: Select All functionality
  const [selectedCustomers, setSelectedCustomers] = useState([]);
  const [selectAll, setSelectAll] = useState(false);

  // FIX 6: Delete confirmation modal
  ...
  // FIX 1: Filter customers based on search term
  const filteredCustomers = emailTable.filter(customer =>
    customer.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    customer.Email.toLowerCase().includes(searchTerm.toLowerCase()) ||
    customer.Phone.toLowerCase().includes(searchTerm.toLowerCase()) ||
    customer.Store.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // FIX 2: Sort customers
  ...
  // FIX 4: Handle individual checkbox
  const handleSelectCustomer = (customerId) => {
    setSelectedCustomers(prev => {
      if (prev.includes(customerId)) {
        ...
        setSelectAll(newSelected.length === sortedCustomers.length);
        return newSelected;
      }
    });
  };
  ...
  // FIX 3: Working Edit functionality
  ...
  // FIX 3: Handle Delete button
  const handleDeleteClick = (customer) => {
    setCustomerToDelete(customer);
    setShowDeleteConfirm(true);
  };

  // FIX 3: Confirm delete
  const confirmDelete = () => {
    setEmailTable(emailTable.filter(c => c.id !== customerToDelete.id));
    setSelectedCustomers(selectedCustomers.filter(id => id !== customerToDelete.id));
    setShowDeleteConfirm(false);
    setCustomerToDelete(null);
  };

  // FIX 5: Add new phone field
  const handleAddPhone = (e) => {
    e.preventDefault();
    setFormPhone([...formPhone, ""]);
  };

  // FIX 5: Remove phone field
  const handleRemovePhone = (index) => {
    if (formPhone.length > 1) {
      setFormPhone(formPhone.filter((_, i) => i !== index));
    }
  };

  // FIX 5: Update phone field
  const handlePhoneChange = (index, value) => {
    const newPhones = [...formPhone];
    newPhones[index] = value;
    setFormPhone(newPhones);
  };

  // FIX 5: Add new email field
  const handleAddEmail = (e) => {
    e.preventDefault();
    setFormEmail([...formEmail, ""]);
  };

  // FIX 5: Remove email field
  const handleRemoveEmail = (index) => {
    if (formEmail.length > 1) {
      setFormEmail(formEmail.filter((_, i) => i !== index));
    }
  };

  // FIX 5: Update email field
  const handleEmailChange = (index, value) => {
    const newEmails = [...formEmail];
    newEmails[index] = value;
    setFormEmail(newEmails);
  };
  ...
  const handleSubmit = () => {
    if (editingCustomer) {
      ...
      setEmailTable(emailTable.map(c =>
      ...
      // Add new customer
      const newCustomer = {
        id: emailTable.length > 0 ? Math.max(...emailTable.map(c => c.id)) + 1 : 1,
        name: formName,
        Email: formEmail[0],
        Phone: formPhone[0],
        Date: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }),
        Store: formStore,
        img: sop1, // Default image
      };
      setEmailTable([...emailTable, newCustomer]);
    }
  ...
  return (
    <>
          ...
          <table className="w-full table">
            <thead>
              <tr className="bg-[#F6F8FA]">
                <th width={42}>
                  {/* FIX 4: Working Select All checkbox */}
                  <Checkbox
                    id="selectAll"
                    checked={selectAll}
              ...
              {sortedCustomers.length > 0 ? (
                sortedCustomers.map((item, index) => (
                  <tr key={item.id} className="!border-b border-[#E2E4E9]">
                    <td className="!py-2">
                      {/* FIX 4: Working individual checkboxes */}
                      <Checkbox
                        id={`select_${item.id}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-5 mb-5">
            ...
            <div className="col-span-2">
              <div className="lg:gap-5 mb-3 md:mb-4 lg:mb-5">
                ...
                {/* FIX 5: Dynamic Phone Fields */}
                {formPhone.map((phone, index) => (
                ...
                {/* FIX 5: Dynamic Email Fields */}
                {formEmail.map((email, index) => (
      ...
      {showDeleteConfirm && (
        <Modal
          innerClass="max-w-[500px]"
          onClick={() => setShowDeleteConfirm(false)}
          closeIconHide={false}
        >
          <div className="text-center">
            <h2 className="text-2xl text-black font-inter font-semibold leading-normal mb-4">
              Confirm Delete
            </h2>
            ...
            <div className="flex gap-3 justify-center">
              <button
                onClick={() => setShowDeleteConfirm(false)}
                className="btn text-primary border-primary px-6"
              >
                Cancel
              </button>
              <button
                onClick={confirmDelete}
                className="btn bg-red-600 text-white px-6 hover:bg-red-700"
              >
                Delete
              </button>
            </div>
