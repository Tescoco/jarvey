...
export default function ArticleRecommendation() {
  ...
  // Check for changes
  useEffect(() => {
    const changed = JSON.stringify(configData) !== JSON.stringify(originalConfigData);
    setHasChanges(changed);
  }, [configData, originalConfigData]);

  // Load configuration from localStorage/API
  const loadConfiguration = () => {
    try {
      // Try to load from localStorage first
      const saved = localStorage.getItem('articleRecommendationConfig');
      if (saved) {
        const parsed = JSON.parse(saved);
        setConfigData(parsed);
        setOriginalConfigData(parsed);
      } else {
        // Set default values
        const defaults = { helpCenter: "jarvey" };
        setConfigData(defaults);
        setOriginalConfigData(defaults);
      }
    } catch (error) {
  ...
  const validateConfiguration = () => {
    ...
    if (!configData.helpCenter || !configData.helpCenter.trim()) {
      errors.helpCenter = 'Help Center name is required';
    } else if (configData.helpCenter.trim().length < 3) {
      errors.helpCenter = 'Help Center name must be at least 3 characters';
    } else if (!/^[a-zA-Z0-9-_]+$/.test(configData.helpCenter)) {
      errors.helpCenter = 'Help Center name can only contain letters, numbers, hyphens, and underscores';
    }
  ...
  const handleSaveConfiguration = async () => {
    ...
    try {
      ...
      localStorage.setItem('articleRecommendationConfig', JSON.stringify(configData));
        ...
        {activeTab === "Configuration" && (
          <div className="flex flex-wrap md:flex-nowrap text-center md:text-start justify-between gap-4">
            <div className="max-w-[400px] xl:max-w-[615px]">
              {/* Save Status Message */}
              {saveMessage && (
                <div className={`mb-6 p-4 rounded-lg flex items-center gap-3 ${saveMessage.type === 'success'
              ...
              <div className="mb-8 md:mb-14 xl:mb-[86px]">
                <Input
                  ...
                  onChange={(e) => handleConfigChange('helpCenter', e.target.value)}
                  onBlur={() => {
                    if (!configData.helpCenter.trim()) {
                      setConfigErrors(prev => ({ ...prev, helpCenter: 'Help Center name is required' }));
                    }
                  }}
                />
                {configErrors.helpCenter && (
                  <p className="text-[#FE4234] text-xs mt-2 flex items-center gap-1">
              ...
              <div className="flex justify-center md:justify-start gap-2 items-center">
                <button
                  className={`btn bg-prim min-w-[123px] px-0 ${isSaving || !hasChanges ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                ...
                <button
                  className={`btn bg-off px-0 min-w-[74px] ${!hasChanges ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
